...
class SyntaxHighlighter():
    ...
    def __traverse__(self, tunit, callback, data):
        def visitor(ast_node, ast_parent_node, data):
            if ast_node.location.file and ast_node.location.file.name == tunit.spelling:  # we're only interested in symbols from associated translation unit
                ast_node_id = self.parser.get_ast_node_id(ast_node)
                if ast_node_id != ASTNodeId.getUnsupportedId():
                    callback(
                        ast_node_id,
                        self.parser.get_ast_node_name(ast_node),
                        self.parser.get_ast_node_line(ast_node),
                        self.parser.get_ast_node_column(ast_node),
                        data
                    )
                else:
        ...
        self.parser.traverse(tunit.cursor, data, visitor)
